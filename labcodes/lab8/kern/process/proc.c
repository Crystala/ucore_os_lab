...
alloc_proc(void) {
     ...
     */
     //LAB5 2013011336 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2013011336 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2013011336 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2013011336
    //LAB8:EXERCISE2 2013011336  HINT:how to copy the fs in parent's proc_struct?
    /*
    ...
    */
    ...
    bool t_interrupt;
    local_intr_save(t_interrupt);
    {
        proc->pid = get_pid();
        hash_proc(proc);
        set_links(proc);
    }
    local_intr_restore(t_interrupt);
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2013011336  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
